    ...
    while(1) {
       ...
       if(!ourSideIsLeft){
           ...
           if(refereeClient->getLastFoul() == VSSRef::Foul::KICKOFF){
               ...
               {
                   int sorteio = rand()%3;
                   double posy[3] = {0.05,0,-0.05};
                   double posx[3] = {0.05,0.1,0.05};
                   double x[3] = {0.7,0.3,posx[sorteio]};
                   double y[3] = {0,0,posy[sorteio]}; //estava 0.05
                   double orient[3] = {-135,0,135};
                   double ori[3] = {90,0,orient[sorteio]}; //estava -135
                   reposicionar_time(x,y,ori,replacerClient);
               }
                if(refereeClient->getLastFoulColor() == VSSRef::Color::BLUE){
                    double x[3] = {0.7,0.5,0.3};
                    double y[3] = {0,0,0};
                    double ori[3] = {90,0,0};
                    reposicionar_time(x,y,ori,replacerClient);
                }
                estrategia.flagTbT = 0;
        ...
        if(lastEnv.has_frame()) {
            ...
            int ultimo_comando = refereeClient->getLastFoul();  // Status de jogo
